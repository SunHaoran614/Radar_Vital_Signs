...
| 参数 | 值 | 说明 |
|------|-----|------|
| **工作频率** | 60-64 GHz | 起始频率60 GHz, 结束频率64 GHz |
| **带宽** | 4 GHz | 频率扫描带宽 |
| **中心频率** | 62 GHz | 中心工作频率 |
| **距离分辨率** | 3.75 cm | c/(2×BW) |
| **最大探测距离** | ~5 m | 理论最大距离 |
| **帧率** | 20 fps | 每秒20帧 |
| **接收天线** | 3 | RX1, RX2, RX3 |
| **发射天线** | 1 | TX1 |
...
```
Dataset/BGT60TR13C_Indoor/
├── Short_Term/                      # 短时测量（8名受试者）
│   ├── Subject_01/
│   │   ├── radar_data.bin          # 雷达原始数据
│   │   ├── heart_rate_ref.csv      # Bigrun心率参考值
│   │   ├── metadata.json           # 元数据（时间、环境等）
│   │   └── config.yaml             # 雷达配置参数
│   ├── Subject_02/
│   │   └── ...
│   ├── ...
...
├── Long_Term/                       # 长时监测（2小时）
│   ├── Subject_01_2h/
│   │   ├── radar_data.bin          # 2小时连续雷达数据
│   │   ├── heart_rate_ref.csv      # 2小时连续心率参考
│   │   ├── metadata.json           # 详细元数据
│   │   ├── config.yaml             # 雷达配置
│   │   └── segments/               # 分段数据（可选）
│   │       ├── segment_00.bin      # 0-10分钟
│   │       ├── segment_01.bin      # 10-20分钟
│   │       └── ...
│
├── README.md                        # 数据集说明
└── data_collection_protocol.pdf    # 数据采集协议
...
- 列:
  - `timestamp`: 时间戳（秒或毫秒）
  - `heart_rate`: 心率值（bpm）
  - `rr_interval`: RR间期（ms，可选）
  - `quality`: 信号质量指标（可选）

示例:
```csv
timestamp,heart_rate,quality
0.0,72,good
1.0,73,good
2.0,72,good
```
...
```json
{
  ...
  "radar_distance_cm": 80,
  "subject_position": "sitting",
  "notes": "Subject remained still during measurement"
}
```
...
radar:
  model: BGT60TR13C
  frequency_start_ghz: 60
  frequency_end_ghz: 64
  bandwidth_ghz: 4
  num_rx: 3
  num_tx: 1
  frame_rate_fps: 20
...
| 受试者ID | 年龄 | 性别 | 身高(cm) | 体重(kg) | 备注 |
|---------|------|------|----------|----------|------|
| Subject_01 | 25 | M | 175 | 70 | 长时监测受试者 |
| Subject_02 | 28 | F | 165 | 55 | - |
| Subject_03 | 30 | M | 180 | 75 | - |
| Subject_04 | 26 | F | 160 | 52 | - |
| Subject_05 | 32 | M | 178 | 80 | - |
| Subject_06 | 24 | F | 168 | 58 | - |
| Subject_07 | 29 | M | 172 | 68 | - |
| Subject_08 | 27 | F | 163 | 54 | - |
...
- **总受试者数**: 8人
- **短时测量**: 8个场景（每人1次，5-10分钟）
- **长时测量**: 1个场景（2小时连续）
- **总数据时长**: 约3小时（短时~1小时 + 长时2小时）
- **总数据量**: 约15-20 GB
- **心率范围**: 60-90 bpm（静息状态）
- **采集时间跨度**: 2024年10-11月
...
| 特性 | 4TU.ResearchD | PhysDrive | BGT60TR13C_Indoor |
|------|---------------|-----------|-------------------|
| **雷达型号** | AWR1642 | 未公开 | BGT60TR13C |
| **频率** | 60 GHz | 60 GHz | 60-64 GHz |
| **受试者数** | 10 | 25 | 8 |
| **场景类型** | 极端生理 | 车载驾驶 | 室内静态 |
| **最长时长** | 60秒 | 数分钟 | **2小时** |
| **参考设备** | Polar H10 | 多模态 | Bigrun |
| **特色** | 极端场景 | 多模态 | **长时监测** |
| **数据量** | ~600 MB/次 | 变化 | ~7 GB（长时） |
...
3. **跨平台研究**:
   - BGT60TR13C vs AWR1642（4TU）的域适应
   - 不同雷达芯片的信号特性对比
   - 跨平台泛化算法开发
...
| 数据集 | 主要特点 | 适用研究方向 |
|--------|----------|-------------|
| **4TU.ResearchD** | 极端生理场景、多场景 | 鲁棒性、极端场景处理 |
| **PhysDrive** | 车载环境、多模态 | 多模态融合、实际应用 |
| **BGT60TR13C_Indoor** | 长时监测、跨平台 | 长期稳定性、跨雷达域适应 |

**三个数据集的协同作用**：
- **4TU**: 提供极端场景的挑战性数据
- **PhysDrive**: 提供真实应用场景的复杂数据
- **BGT60TR13C**: 提供长时稳定性和跨平台验证数据
